...
# KasmVNC Auto-Install Script for Ubuntu Noble (24.04) and Derivatives
# Created: July 4, 2025
...
# Install required dependencies for Ubuntu Noble
...
# KasmVNC download URL for Noble (update version as needed)
KASMVNC_VERSION="1.0.3"
if [ "$ARCH" = "amd64" ]; then
    # Try Noble package first, fallback to Jammy if Noble not available
    KASMVNC_URL="https://github.com/kasmtech/KasmVNC/releases/download/v${KASMVNC_VERSION}/kasmvncserver_jammy_${KASMVNC_VERSION}_amd64.deb"
elif [ "$ARCH" = "arm64" ]; then
    KASMVNC_URL="https://github.com/kasmtech/KasmVNC/releases/download/v${KASMVNC_VERSION}/kasmvncserver_jammy_${KASMVNC_VERSION}_arm64.deb"
else
...
if ! wget "$KASMVNC_URL" -O kasmvnc.deb; then
    warn "Noble-specific package not available, trying Jammy package as fallback..."
    # This should work on Noble as it's generally compatible
fi
...
# Create optimized xstartup file for Ubuntu Noble
log "Creating optimized xstartup configuration for Ubuntu Noble..."
...
# Ubuntu Noble (24.04) specific environment variables
...
# Initialize audio system for Ubuntu Noble
...
echo
echo -e "${YELLOW}Ubuntu Noble (24.04) Specific Notes:${NC}"
echo "• This version includes PipeWire/Wayland compatibility settings"
echo "• Audio system is initialized for better multimedia support"
echo "• Uses X11 backend explicitly for VNC compatibility"
echo
...
echo "  ✅ Audio support (PipeWire/PulseAudio)"
echo
...
# Ubuntu Noble specific tips
echo
echo -e "${BLUE}Ubuntu Noble (24.04) Tips:${NC}"
echo "• If you experience any Wayland-related issues, the script forces X11 mode"
echo "• For best performance, ensure hardware acceleration is available"
echo "• PipeWire audio should work out of the box for multimedia applications"
